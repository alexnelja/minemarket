...
-- Listings: price and volume must be positive
ALTER TABLE listings
  ADD CONSTRAINT chk_listings_price_positive
  CHECK (price_per_tonne > 0);

ALTER TABLE listings
  ADD CONSTRAINT chk_listings_volume_positive
  CHECK (volume_tonnes > 0);

-- Requirements: target price and volume must be positive
ALTER TABLE requirements
  ADD CONSTRAINT chk_requirements_price_positive
  CHECK (target_price > 0);

ALTER TABLE requirements
  ADD CONSTRAINT chk_requirements_volume_positive
  CHECK (volume_needed > 0);

-- Deals: agreed price and volume must be positive
ALTER TABLE deals
  ADD CONSTRAINT chk_deals_price_positive
  CHECK (agreed_price > 0);

ALTER TABLE deals
  ADD CONSTRAINT chk_deals_volume_positive
  CHECK (volume_tonnes > 0);

-- Deals: escrow amount must be non-negative when set
ALTER TABLE deals
  ADD CONSTRAINT chk_deals_escrow_non_negative
  CHECK (escrow_amount IS NULL OR escrow_amount >= 0);

-- Deals: fx_rate must be positive when set
ALTER TABLE deals
  ADD CONSTRAINT chk_deals_fx_rate_positive
  CHECK (fx_rate_locked IS NULL OR fx_rate_locked > 0);

-- Routes: distance must be positive
ALTER TABLE routes
  ADD CONSTRAINT chk_routes_distance_positive
  CHECK (distance_km > 0);

-- Listings: max_buyers must be positive when set
ALTER TABLE listings
  ADD CONSTRAINT chk_listings_max_buyers_positive
  CHECK (max_buyers IS NULL OR max_buyers > 0);

-- Deals: buyer and seller must be different users
ALTER TABLE deals
  ADD CONSTRAINT chk_deals_different_parties
...
-- Harbours: name should be unique (prevent duplicate harbour entries)
CREATE UNIQUE INDEX IF NOT EXISTS uq_harbours_name
  ON harbours(name);
